@if (!$cohort['user_has_submitted'])
    <div class="w-full px-8 py-4 my-4 duration-300 bg-white rounded-lg shadow-lg hover:shadow-xl"
        x-data="{showcreatebox: false}" x-bind:class="{'cursor-pointer':!showcreatebox}"
...
@else
    ...
    <div class="w-full px-8 py-4 mb-4 bg-white rounded-lg rounded-t-none shadow-lg"
        x-data="{showreply: true, reply: ''}">
        ...
        <div class="flex items-center pt-6">
            ...
            <button x-on:click="showreply = !showreply"
                class="h-6 px-2 ml-4 text-sm leading-7 text-purple-800 text-opacity-75 cursor-pointer hover:text-opacity-100">
                <span x-show="showreply">
                    hide replies
                </span>
                <span x-show="!showreply">
                    show replies
...
<div id="navigator" class="flex justify-between my-4">
    <a href="{{ '/' . $cohort->name . '/week/' . ($week - 1) }}" class="px-4 py-1 bg-white rounded rounded-full">
        << prev</a>
            <a href="{{ '/' . $cohort->name . '/week/' . ($week + 1) }}"
                class="px-4 py-1 bg-white rounded rounded-full">next
                >></a>
</div>
@foreach ($summaries as $key => $summary)
    @if (!$summary['is_own'])
        <div class="w-full px-8 py-4 my-4 bg-white rounded-lg shadow-lg" x-data="{showreply: true, reply: ''}">
            <h2 class="text-xl font-black">Last Week's Goals</h2>
            <p class="mb-4 text-lg text-gray-800">
                {{ $summary['last_goal'] }}
            </p>
